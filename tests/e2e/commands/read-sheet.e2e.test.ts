...
describe('Read Sheet E2E', () => {
  ...
  it('should read a sheet tab content', async () => {
    const listTabsResult = await execCommand('sheet list', undefined, 15000, testHomeDir);
    ...
    await execCommand(`sheet write -n "${tabName}" -c A1 -v "Test Content"`, undefined, 15000, testHomeDir);

    const readResult = await execCommand(`sheet read -n "${tabName}"`, undefined, 15000, testHomeDir);
  ...
  it('should handle non-existent tab gracefully', async () => {
    const readResult = await execCommand('sheet read -n "NonExistentTab123"', undefined, 15000, testHomeDir);
