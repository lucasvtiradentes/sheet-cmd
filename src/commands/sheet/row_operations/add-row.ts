    ...
    async (options: RowAddOptions) => {
      try {
        ...
        if (options.below) {
          ...
          inheritFromBefore = options.formulas ? false : false;
        } else {
          insertPosition = rowNumber - 1;
          sourceRowForFormulas = rowNumber - 1;
          inheritFromBefore = options.formulas ? true : false;
        }

        const rowWord = count === 1 ? 'row' : 'rows';
        Logger.loading(`Adding ${count} ${rowWord} ${options.below ? 'below' : 'above'} row ${rowNumber}...`);

        await sheetsService.insertRows(sheetName, {
        ...
        if (options.formulas) {
          ...
          for (let i = 0; i < count; i++) {
            await sheetsService.copyRowFormulas(sheetName, sourceRowForFormulas, insertPosition + i);
          }
        }
